...
função quadrática. Ajuste de ganho da função quadrática. Ajuste do limite de
velocidade da roda que está no exterior da curva. Botão 3 (PB3) entra no menu de
...
int vCurve = 4;  // Curve outside wheel max speed limit <> Limite de velocidade
...
void loop() {
  ...
  if (line <= 0) {
    velM1 =
        (int)((double)vel * cos(((double)line /
                                 kLimit)));  // Quadratic function for Motor1 <>
                                             // Função quadrática para o Motor1
    velM2 = vel + (vel - velM1);
  } else {
    velM2 =
        (int)((double)vel * cos(((double)line /
                                 kLimit)));  // Quadratic function for Motor2 <>
                                             // Função quadrática para o Motor2
    velM1 = vel + (vel - velM2);
  }
