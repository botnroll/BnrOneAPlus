 ...
 * velocidade da roda que está no exterior da curva. Botão 3 (PB3) entra no
 * menu de configuração do controlo.
 *
...
double gain = 40.0;   // Quadratic function gain <> Ganho da função quadrática
int extra_speed = 4;  // Curve outside wheel max speed limit <> Limite de
...
void menu() {
  ...
  //**** Quadratic function gain gain <> Ganho gain da funcão quadrática****
  temp = gain * 1.0;
  temp_var = (int)temp;
  while (button != 3) {
...
void loop() {
  ...
  // Quadratic function for Motor1 <> Função quadrática para o Motor1
  if (line <= 0) {
    velM1 = (int)((double)speed * cos(((double)line / gain)));
    velM2 = speed + (speed - velM1);
  }
  // Quadratic function for Motor2 <> Função quadrática para o Motor2
  else {
    velM2 = (int)((double)speed * cos(((double)line / gain)));
    velM1 = speed + (speed - velM2);
  }
