...
Description:
This program detects automatic start and does the automatic end on the RoboParty
Fun Challenge.
*/
...
void setup() {
  ...
  initialize_timer();     // configures the interrupt timer for the end of the
                          // challenge
...
ISR(TIMER1_COMPA_vect) {  // timer1 interrupt 1Hz
  ...
  if (counter >= CHALLENGE_TIME) {
    ...
    while (
        1)  // does not allow anything else to be done after the challenge ends
    {
