...
private:
    void VerifyAndCorrectArray(int* array, const int min, const int max, const int defaultValue);

    template < class T >
    void VeriyAndCorrectValue(T& value, const T min, const T max, const T defaultValue);

    byte LoadArrayValues(byte eepromADD, int array[8]) const;

    byte LoadWord(byte eepromADD, int& outValue) const;

    byte LoadByte(byte eepromADD, int& outValue) const;

    void PrintArray(String text, int array[8]) const;

    void PrintValue(String text, int value) const;

    byte SaveArrayValues(byte eepromADD, int array[8]) const;

    byte SaveWord(byte eepromADD, int value) const;

    byte SaveByte(byte eepromADD, int value) const;
